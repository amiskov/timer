	...
	return _Utils_Tuple2(
		{
			...
			timer: {counter: 1, currentRound: 1, rest: 2, rounds: 2, work: 4}
		},
...
var $author$project$Main$updateTimer = F2(
	function (nextRound, t) {
		return _Utils_update(
			...
			{
				counter: t.counter + 1,
				currentRound: nextRound ? (t.currentRound + 1) : t.currentRound
			});
	});
...
var $author$project$Main$updateTimerSetting = F3(
	function (timer, updater, newValue) {
		var newVal = A2(
			$elm$core$Maybe$withDefault,
			0,
			$elm$core$String$toInt(newValue));
		return A2(updater, timer, newVal);
	});
		...
		switch (msg.$) {
			case 'RunTimer':
				return _Utils_Tuple2(
					...
					$elm$core$Platform$Cmd$none);
			case 'TogglePause':
				var _v1 = A2($elm$core$Debug$log, 'paused 222', model.step);
				var _v2 = model.step;
				if (_v2.$ === 'Paused') {
					var s = _v2.a;
					return _Utils_Tuple2(
			...
			default:
				...
				switch (_v3.$) {
					...
					case 'Running':
						var phase = _v3.a;
						var singleRoundTime = model.timer.work + model.timer.rest;
						var totalTime = singleRoundTime * model.timer.rounds;
						var nextRound = (model.timer.counter > 0) && (!(model.timer.counter % singleRoundTime));
						var currentRoundTime = model.timer.counter % singleRoundTime;
						var currentPhaseTime = function () {
							if (phase.$ === 'Work') {
								return currentRoundTime % model.timer.work;
							} else {
								return currentRoundTime % model.timer.rest;
							}
						}();
						var _v4 = A2(
							$elm$core$Debug$log,
							'round, phase',
							_Utils_Tuple2(currentRoundTime, currentPhaseTime));
						return _Utils_eq(model.timer.counter, totalTime - model.timer.rest) ? _Utils_Tuple2(
							_Utils_update(
								model,
							...
							_Utils_update(
								model,
								{
									step: function () {
										if (phase.$ === 'Work') {
											return ((!currentPhaseTime) && (!(!model.timer.counter))) ? $author$project$Main$Running(
												$author$project$Main$Rest(currentPhaseTime)) : $author$project$Main$Running(
												$author$project$Main$Work(currentPhaseTime));
										} else {
											return (!currentPhaseTime) ? $author$project$Main$Running(
												$author$project$Main$Work(currentPhaseTime)) : $author$project$Main$Running(
												$author$project$Main$Rest(currentPhaseTime));
										}
									}(),
									timer: A2($author$project$Main$updateTimer, nextRound, model.timer)
								}),
							$elm$core$Platform$Cmd$none);
					default:
...
var $author$project$Main$viewRunningTimer = F2(
	function (model, _v0) {
		...
		var _v1 = model.timer;
		var work = _v1.work;
		var rounds = _v1.rounds;
		var currentRound = _v1.currentRound;
		var phaseText = phase + (': ' + $elm$core$String$fromInt(
			(!phaseTime) ? work : phaseTime));
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
				...
				[
					...
					_List_fromArray(
						[
							...
							'phase_' + $elm$core$String$toLower(phase))
						]),
					_List_fromArray(
						[
							A2(
							...
							_List_fromArray(
								[
									$elm$html$Html$Attributes$class('phase-title')
								]),
							_List_fromArray(
								[
									$elm$html$Html$text(phaseText)
								])),
							A2(
							$elm$html$Html$p,
							_List_fromArray(
							...
							_List_fromArray(
								[
									$elm$html$Html$text(
									'Round ' + ($elm$core$String$fromInt(currentRound) + (' of ' + $elm$core$String$fromInt(rounds))))
								]))
						])),
					A2(
					$elm$html$Html$button,
...
var $author$project$Main$viewRenderOptions = F2(
	function (count, selectedOptionNum) {
		return A2(
			$elm$core$List$map,
			function (n) {
				return A2(
...
var $author$project$Main$viewTimerForm = function (_v0) {
	...
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Events$onSubmit($author$project$Main$ShowCountdown)
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						...
						A2($author$project$Main$viewRenderOptions, 20, rounds))
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('row row_work')
					]),
				_List_fromArray(
					[
						A2(
						...
						A2($author$project$Main$viewRenderOptions, 240, work))
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('row row_rest')
					]),
				_List_fromArray(
					[
						A2(
						...
						A2($author$project$Main$viewRenderOptions, 120, rest))
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('row row_btn row_go')
					]),
				_List_fromArray(
					[
						A2(
					...
					case 'Countdown':
						return A2(
							...
							_List_fromArray(
								[
									$elm$html$Html$text(
									(model.countdown > 0) ? $elm$core$String$fromInt(model.countdown) : 'Go!')
								]));
					default:
						var phase = _v0.a;
						var phaseData = function () {
							if (phase.$ === 'Work') {
								var t = phase.a;
								return _Utils_Tuple2('Work', model.timer.work - t);
							} else {
								var t = phase.a;
								return _Utils_Tuple2('Rest', model.timer.rest - t);
							}
						}();
						return A2($author$project$Main$viewRunningTimer, model, phaseData);
				}
			}()
			]));
};
